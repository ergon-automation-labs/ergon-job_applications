defmodule BotArmyJobApplications.FormatterTest do
  ...
  describe "format/2" do
    test "opportunity_discovered with title and company" do
      result =
        Formatter.format(:opportunity_discovered, %{
          "title" => "Senior Engineer",
          "company" => "Acme Corp"
        })

      assert result ==
               "◆ Senior Engineer at Acme Corp just hit your filter. This one's worth your time."
    end

    test "application_submitted" do
      result =
        Formatter.format(:application_submitted, %{
    ...
    test "interview_scheduled" do
      result =
        Formatter.format(:interview_scheduled, %{
      ...
      assert result ==
               "◆ Interview locked: Principal Engineer at FutureTech on 2026-04-01. You've got this."
    end

    test "interview_feedback" do
      result =
        Formatter.format(:interview_feedback, %{
          "title" => "VP Engineering",
          "feedback" => "Strong technical depth, good communication"
        })

      assert result ==
               "◆ Interview notes for VP Engineering: Strong technical depth, good communication"
    end

    test "offer_received" do
      result =
        Formatter.format(:offer_received, %{
          "title" => "CTO",
          "company" => "StartupXYZ"
        })
