defmodule BotArmyJobApplications.Repo.Migrations.AddTenantAndUserId do
  ...
  def up do
    ...
    alter table(:resumes) do
      add(:tenant_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:resumes, :tenant_id)
      add(:user_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:resumes, :user_id)
    rescue
      _ -> nil
    end

    create(index(:resumes, [:tenant_id])) unless Ecto.Migration.index_exists?(:resumes, [:tenant_id])
    create(index(:resumes, [:user_id])) unless Ecto.Migration.index_exists?(:resumes, [:user_id])

    execute(
      "UPDATE resumes SET tenant_id = '#{default_tenant_id}'::uuid WHERE tenant_id IS NULL"
    ) unless Ecto.Migration.column_exists?(:resumes, :tenant_id)

    # Add tenant_id and user_id to skills (resume_skills) (idempotent)
    alter table(:resume_skills) do
      add(:tenant_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:resume_skills, :tenant_id)
      add(:user_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:resume_skills, :user_id)
    rescue
      _ -> nil
    end

    create(index(:resume_skills, [:tenant_id])) unless Ecto.Migration.index_exists?(:resume_skills, [:tenant_id])
    create(index(:resume_skills, [:user_id])) unless Ecto.Migration.index_exists?(:resume_skills, [:user_id])

    execute(
      "UPDATE resume_skills SET tenant_id = '#{default_tenant_id}'::uuid WHERE tenant_id IS NULL"
    ) unless Ecto.Migration.column_exists?(:resume_skills, :tenant_id)

    # Add tenant_id and user_id to roles (resume_roles) (idempotent)
    alter table(:resume_roles) do
      add(:tenant_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:resume_roles, :tenant_id)
      add(:user_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:resume_roles, :user_id)
    rescue
      _ -> nil
    end

    create(index(:resume_roles, [:tenant_id])) unless Ecto.Migration.index_exists?(:resume_roles, [:tenant_id])
    create(index(:resume_roles, [:user_id])) unless Ecto.Migration.index_exists?(:resume_roles, [:user_id])

    execute(
      "UPDATE resume_roles SET tenant_id = '#{default_tenant_id}'::uuid WHERE tenant_id IS NULL"
    ) unless Ecto.Migration.column_exists?(:resume_roles, :tenant_id)

    # Add tenant_id and user_id to bullets (resume_bullets) (idempotent)
    alter table(:resume_bullets) do
      add(:tenant_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:resume_bullets, :tenant_id)
      add(:user_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:resume_bullets, :user_id)
    rescue
      _ -> nil
    end

    create(index(:resume_bullets, [:tenant_id])) unless Ecto.Migration.index_exists?(:resume_bullets, [:tenant_id])
    create(index(:resume_bullets, [:user_id])) unless Ecto.Migration.index_exists?(:resume_bullets, [:user_id])

    execute(
      "UPDATE resume_bullets SET tenant_id = '#{default_tenant_id}'::uuid WHERE tenant_id IS NULL"
    ) unless Ecto.Migration.column_exists?(:resume_bullets, :tenant_id)

    # Add tenant_id and user_id to listings (idempotent)
    alter table(:listings) do
      add(:tenant_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:listings, :tenant_id)
      add(:user_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:listings, :user_id)
    rescue
      _ -> nil
    end

    create(index(:listings, [:tenant_id])) unless Ecto.Migration.index_exists?(:listings, [:tenant_id])
    create(index(:listings, [:user_id])) unless Ecto.Migration.index_exists?(:listings, [:user_id])

    execute(
      "UPDATE listings SET tenant_id = '#{default_tenant_id}'::uuid WHERE tenant_id IS NULL"
    ) unless Ecto.Migration.column_exists?(:listings, :tenant_id)

    # Add tenant_id and user_id to applications (idempotent)
    alter table(:applications) do
      add(:tenant_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:applications, :tenant_id)
      add(:user_id, :uuid, null: true) unless Ecto.Migration.column_exists?(:applications, :user_id)
    rescue
      _ -> nil
    end

    create(index(:applications, [:tenant_id])) unless Ecto.Migration.index_exists?(:applications, [:tenant_id])
    create(index(:applications, [:user_id])) unless Ecto.Migration.index_exists?(:applications, [:user_id])

    execute(
      "UPDATE applications SET tenant_id = '#{default_tenant_id}'::uuid WHERE tenant_id IS NULL"
    ) unless Ecto.Migration.column_exists?(:applications, :tenant_id)
  end
